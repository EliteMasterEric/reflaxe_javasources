...
class JavaExpression extends JavaBase {
	...
	**/
	...
	public function compile(expr:TypedExpr, topLevel:Bool):Null<String> {
		...
		switch (expr.expr) {
			case TConst(constant):
				result = compileConstant(constant);
			case TLocal(v):
			...
			case TUnop(op, postFix, e):
				result = compileUnop(op, e, postFix);
			case TBinop(op, e1, e2):
				result = compileBinop(op, e1, e2);
			case TField(e, fa):
				result = compileFieldAccess(e, fa);
			case TTypeExpr(m):
			...
			case TReturn(maybeExpr):
				result = (maybeExpr != null) ? 'return ${_compileExpression(maybeExpr)}' : 'return';

			case TParenthesis(e):
				if (!EverythingIsExprSanitizer.isBlocklikeExpr(e)) {
					result = '(${_compileExpression(e)})';
				} else {
					result = _compileExpression(e);
				}
			...
			case TCall(e, el):
				...
				if (nfc != null) {
					...
					result = '${_compileExpression(e)}($arguments)';
				}
			...
			case TNew(classTypeRef, _, el):
				...
				if (nfc != null) {
					...
					final className = compiler.compileClassName(classTypeRef.get(), true);
	...
	function injectLineNumber(expr:TypedExpr):Void {
		compiler.injectExpressionPrefixContent('//line ${expr.pos.line()} "${expr.pos.getFile()}"');
	}
		...
		return if (nameMeta.hasMeta(":native")) {
			...
			switch (fa) {
				case FStatic(clsRef, cfRef): {
						final cf = cfRef.get();
						final className = compiler.compileClassName(clsRef.get(), true);
